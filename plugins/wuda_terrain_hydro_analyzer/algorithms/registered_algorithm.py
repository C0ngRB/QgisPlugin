...
class RegisteredTerrainHydroAlgorithm(QgsProcessingAlgorithm):
    ...
    IMPLEMENTED_ALGORITHMS = {"check_saga_provider", "run_terrain_workflow", "run_hydrology_workflow"}
    ...
    def shortHelpString(self):
        ...
        return "当前阶段提供插件注册、SAGA 检测入口和运行摘要契约；真实地形水文分析按后续阶段在 core 中补齐。"
    ...
    def processAlgorithm(self, parameters, context, feedback):
        """函数含义：执行当前阶段已开放的地形水文入口；上游由 QGIS Processing 运行器调用；下游生成 run_summary.json 或显式拒绝未实现算法；风险点是不得生成假空间分析结果。"""
