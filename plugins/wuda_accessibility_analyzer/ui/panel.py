...
from core.registry.algorithms import ACCESSIBILITY_PROVIDER_ID, algorithm_display_name, provider_algorithms


class WudaAccessibilityPanel(QDialog):
    """类含义：可达性插件向导入口；上游由插件菜单打开；下游调用 QGIS 原生 Processing 算法窗口；风险点是不能在 UI 中直接执行 processing.run。"""

    def __init__(self, iface):
        """函数含义：构建 workflow 和高级算法按钮；上游由 plugin.open_panel 传入 iface；下游按按钮打开算法窗口；风险点是算法 ID 必须来自 core 注册表。"""
        super().__init__(iface.mainWindow())
        self.iface = iface
        self.setWindowTitle("WUDA 可达性分析")
    ...
    def render_buttons(self):
        ...
        workflow_names = ("run_standardization_workflow", "run_accessibility_workflow")
        algorithm_names = provider_algorithms(ACCESSIBILITY_PROVIDER_ID) if self.mode_checkbox.isChecked() else workflow_names
